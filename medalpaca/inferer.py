...
class MedAlapaca:
    ...
    def __init__(self, modelname: str, prompt_template: str) -> None:
        ...
        self.tokenizer = LlamaTokenizer.from_pretrained(
            self.available_models[modelname]["base_model"]
        )
        ...
        **kwargs,
        ...
        Args:
            ...
            top_p (float, optional):
                Nucleus sampling probability threshold. Controls diversity of output.
            top_k (int, optional):
                Top-k sampling. Controls the number of candidates considered for sampling.
            num_beams (int, optional):
                Number of beams for beam search.
                Controls the number of alternative sequences considered.
            max_new_tokens (int, optional):
                Maximum number of new tokens to generate in the response.
                verbose (bool, optional): If True, print the prompt before generating a response.
            **kwargs:
                Additional keyword arguments to pass to the `GenerationConfig`.

        Returns:
            str: The generated response from the medAlpaca model.
        """

        prompt = self.prompt_template["prompt_input"].format(instruction=instruction, input=input)
        if verbose:
            print(prompt)
