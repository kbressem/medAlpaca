#!/bin/bash
#SBATCH --job-name=alpaca-13-deepspeed  # Specify job name
...
#SBATCH --output=logs_alp-13.o%j        # File name for standard output
#SBATCH --error=errors_alp-13.e%j       # File name for standard error output
...
deepspeed --num_nodes ${SLURM_JOB_NUM_NODES} --num_gpus 8 main.py \
    ...
    --weight_decay 0.1 \
    --num_train_epochs 10 \
    --wandb_run_name 'medalpaca-13b' \
    --lr_scheduler_type cosine \
    --zero_stage 3 \
    --output_dir "/sc-projects/sc-proj-cc06-medbert/alpaca-zoo/in-training/medalpaca-deepspeed-13b" \
